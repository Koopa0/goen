...
The schema carries 230 `CHECK` constraints, 80 foreign keys, 60 unique indexes,
and 39 rule triggers (beside 15 that only keep `updated_at` truthful), and it
...
- **A single writer for money and stock.** A customer-facing request runs as
  ...
  happen only through 18 `SECURITY DEFINER` functions, so there is no second path
