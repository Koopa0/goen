...
func TestStartupRefusesATOTPKeyThatIsNotAKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cfg.TOTPKey = tt.raw
			err := cfg.checkProductionPosture(slog.New(slog.DiscardHandler))
			if tt.wantErr {
				if err == nil || !strings.Contains(err.Error(), "GOEN_TOTP_KEY") {
					t.Fatalf("checkProductionPosture error = %v, want GOEN_TOTP_KEY refusal", err)
				}
			...
			if err != nil {
				t.Fatalf("checkProductionPosture: %v", err)
			}
...
func TestAProductionPostureRefusesAnUnconfiguredMailer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := cfg.checkProductionPosture(log)
			if tt.wantErr {
				if err == nil || !strings.Contains(err.Error(), "GOEN_SMTP_ADDR") {
					t.Fatalf("checkProductionPosture error = %v, want GOEN_SMTP_ADDR refusal", err)
				}
				return
			}
			if err != nil {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantError != "" {
				if err == nil || !strings.Contains(err.Error(), tt.wantError) {
					t.Fatalf("checkProductionPosture error = %v, want %q", err, tt.wantError)
				}
...
func TestPostureReportsTheFirstBrokenDependency(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := tt.cfg.checkProductionPosture(slog.New(slog.DiscardHandler))
			if err == nil || !strings.Contains(err.Error(), tt.want) {
				t.Errorf("checkProductionPosture error = %v, want %q", err, tt.want)
			}
		})
	}
}
...
func TestAMalformedSMTPAddressIsFatalAndNeverTheLogSender(t *testing.T) {
	...
	if bad != nil {
		t.Errorf("newSender malformed returned %T, want nil (never LogSender)", bad)
	}
	...
	if logBuffer.Len() != 0 {
		t.Errorf("newSender duplicated the posture warning: %s", logBuffer.String())
	}
