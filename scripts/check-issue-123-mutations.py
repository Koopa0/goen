...
CASES = [{'name': 'audit',
  ...
  'before': 'auditIn(ctx, q, Event{Action: action, Table: "orders", ID: nullableID(prior.ID)})',
  'after': 'func(context.Context, *db.Queries, Event) error { return nil }(ctx, q, Event{Action: action, Table: '
           '"orders", ID: nullableID(prior.ID)})',
  'test': 'TestStaffNoteHTTPRecordsOperationsWithoutContent',
  'marker': 'order.note.create audit count = 0, want 1',
  'package': './internal/admin',
  'integration': True},
 {'name': 'private-content',
  'path': 'internal/admin/store.go',
  'before': 'Event{Action: action, Table: "orders", ID: nullableID(prior.ID)}',
  'after': 'Event{Action: action, Table: "orders", ID: nullableID(prior.ID), After: map[string]string{"note": note}}',
  'test': 'TestStaffNoteHTTPRecordsOperationsWithoutContent',
  'marker': 'content-free=false',
  'package': './internal/admin',
  'integration': True},
 {'name': 'rollback',
  'path': 'internal/admin/store.go',
  ...
  'marker': 'audit failure persisted note "unrecordable replacement"',
  'package': './internal/admin',
  'integration': True}]
...
os.chdir(repo)
proof = Path(os.environ.get("RUNNER_TEMP", tempfile.gettempdir())) / "goen-issue-123-mutations"
