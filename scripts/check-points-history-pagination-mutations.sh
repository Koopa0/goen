...
proof_dir="${RUNNER_TEMP:?}/goen-points-history-pagination-mutations"
mkdir -p "$proof_dir"
backup_dir=$(mktemp -d)
proof_paths=(internal/loyalty/query.sql internal/ui/pages/points.templ)
...
integration_test='TestPointsPaginationKeepsWholeSpendsAndOlderClawbacksReachable'
integration_children=''
render_test='TestEmptyOlderPointsPageOffersLatestEntries'
render_children=''
cursor_test='TestHistoryCursorKeepsOwnerAndExactGroupBoundary'
...
feature = 'github.com/koopa0/goen/internal/loyalty'
...
run_green() {
  local phase=$1
  local test_status=0
  go test -json -tags=integration -race -count=1 -timeout=5m ./internal/loyalty ./internal/ui/pages \
...
run_green baseline
for mutant in cursor grouping next first; do
  restore_sources
...
mutant = sys.argv[1]
if mutant == 'cursor':
    p = Path('internal/loyalty/query.sql')
    text = p.read_text()
...
elif mutant == 'grouping':
    p = Path('internal/loyalty/query.sql')
    text = p.read_text()
    before = 'GROUP BY entry_key, e.kind, e.reason, e.order_id'
    if text.count(before) != 1:
        raise SystemExit('ledger group mutation target is not unique')
    p.write_text(text.replace(before, before + ', e.id', 1))
else:
    p = Path('internal/ui/pages/points.templ')
    text = p.read_text()
    field = 'v.HistoryNext' if mutant == 'next' else 'v.HistoryFirst'
...
MUTATE
  if [[ "$mutant" == cursor || "$mutant" == grouping ]]; then
    ...
    if [[ "$mutant" == cursor ]]; then
      ...
      grep -F 'GROUP BY entry_key, e.kind, e.reason, e.order_id, e.id' internal/db/query.sql.go > "$proof_dir/$mutant-generated-target.txt"
    fi
  else
    go tool templ generate -path internal/ui > "$proof_dir/$mutant-generation.log" 2>&1
    field='v.HistoryNext'
    if [[ "$mutant" == first ]]; then field='v.HistoryFirst'; fi
    grep -F "if $field != \"\" && false {" internal/ui/pages/points_templ.go > "$proof_dir/$mutant-generated-target.txt"
  fi
  git diff -- "${proof_paths[@]}" > "$proof_dir/$mutant.patch"
  test_package=loyalty
  test_name=$integration_test
  test_children=$integration_children
  assertion='group repeated or split:'
  if [[ "$mutant" == cursor ]]; then
    assertion='new entry displaced the next ledger page'
  fi
  if [[ "$mutant" == next ]]; then
    assertion='ledger lost its plain next link'
  elif [[ "$mutant" == first ]]; then
    test_package=ui/pages
    test_name=$render_test
    test_children=$render_children
    assertion='empty older ledger lost its restart door'
  fi
  test_status=0
...
run_green restored
echo 'points history pagination sources restored exactly; named scoped tests passed.'
