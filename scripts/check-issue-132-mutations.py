#!/usr/bin/env python3
CASES = [{'name': 'staff-cancellation',
  'path': 'internal/admin/store.go',
  'before': 'return ordernotice.Enqueue(ctx, q, e.orderID, ordernotice.CancelledByStaff)',
  'after': 'return nil',
  'test': 'TestTerminalCancellationNoticesFollowCommittedActor',
  'marker': 'missing terminal notice: no rows in result set',
  'package': './internal/admin',
  'integration': True},
 {'name': 'customer-cancellation',
  'path': 'internal/cart/cancel.go',
  'before': 'if err := ordernotice.Enqueue(ctx, q, order.ID, ordernotice.CancelledByCustomer); err != nil {',
  'after': 'if err := func(_ ordernotice.Kind) error { return nil }(ordernotice.CancelledByCustomer); err != nil {',
  'test': 'TestTerminalCancellationNoticesFollowCommittedActor',
  'marker': 'missing terminal notice: no rows in result set',
  'package': './internal/admin',
  'integration': True},
 {'name': 'arrival',
  'path': 'internal/admin/store.go',
  'before': 'return ordernotice.Enqueue(ctx, q, e.orderID, kind)',
  'after': '_ = kind\n\t\treturn nil',
  'test': 'TestTerminalArrivalIsNotRequeuedAfterCompletionOrOutboxRetention',
  'marker': 'missing terminal notice: no rows in result set',
  'package': './internal/admin',
  'integration': True},
 {'name': 'erased-recipient',
  'path': 'cmd/goen/main.go',
  'before': 'if errors.Is(err, pgx.ErrNoRows) {\n'
            '\t\t\treturn nil\n'
            '\t\t}\n'
            '\t\tif err != nil {\n'
            '\t\t\treturn fmt.Errorf("read terminal order recipient: %w", err)',
  'after': 'if errors.Is(err, pgx.ErrNoRows) {\n'
           '            return '
           'notifier.SendOrderTerminal(ctx,p.Kind,email.TerminalRecipient{Address:"erased@example.com",Name:"Erased",Locale:"en",OrderNumber:"erased"})\n'
           '\t\t}\n'
           '\t\tif err != nil {\n'
           '\t\t\treturn fmt.Errorf("read terminal order recipient: %w", err)',
  'test': 'TestTerminalWorkerSkipsAnAddressErasedBeforeDelivery',
  'marker': 'worker restored an erased or missing recipient',
  'package': './cmd/goen',
  'integration': True},
 {'name': 'duplicate-arrival',
  'path': 'internal/admin/store.go',
  'before': '} else if e.previous == pages.FulfillmentDelivered {',
  'after': '} else if false && e.previous == pages.FulfillmentDelivered {',
  'test': 'TestTerminalArrivalIsNotRequeuedAfterCompletionOrOutboxRetention',
  'marker': 'completion sent a second arrival after retention',
  'package': './internal/admin',
  'integration': True}]
...
os.chdir(repo)
proof = Path(os.environ.get("RUNNER_TEMP", tempfile.gettempdir())) / "goen-issue-132-mutations"
