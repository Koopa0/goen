...
proof_dir="${RUNNER_TEMP:?}/goen-admin-pagination-mutations"
mkdir -p "$proof_dir"
backup_dir=$(mktemp -d)
proof_paths=(internal/admin/query.sql internal/ui/pages/adminlist.templ)
...
integration_test='TestEveryAdminQueueReachesBeyondItsFirstPage'
integration_children='orders|order_search|customers|products|stock|movements|returns|coupons|campaigns|reviews|messages|credit|audit|warranty'
render_test='TestEveryAdminEmptyPageKeepsItsRestartLink'
render_children='orders|customers|products|stock|movements|returns|coupons|campaigns|reviews|messages|credit|audit|warranty'
cursor_test='TestPageCursorKeepsItsQueueAndFilters'
...
feature = 'github.com/koopa0/goen/internal/admin'
...
run_green() {
  local phase=$1
  local test_status=0
  go test -json -tags=integration -race -count=1 -timeout=5m ./internal/admin ./internal/ui/pages \
...
MUTATE
  if [[ "$mutant" == cursor ]]; then
    ...
    [[ "$(wc -l < "$proof_dir/$mutant-generated-target.txt")" -eq 14 ]]
  else
    go tool templ generate -path internal/ui > "$proof_dir/$mutant-generation.log" 2>&1
    field='b.Next'
    if [[ "$mutant" == first ]]; then field='b.First'; fi
    grep -F "if $field != \"\" && false {" internal/ui/pages/adminlist_templ.go > "$proof_dir/$mutant-generated-target.txt"
  fi
  git diff -- "${proof_paths[@]}" > "$proof_dir/$mutant.patch"
  test_package=admin
  test_name=$integration_test
  test_children=$integration_children
  assertion='row repeated across pages:'
  if [[ "$mutant" == next ]]; then
    assertion='rendered page lost the next cursor'
  elif [[ "$mutant" == first ]]; then
    test_package=ui/pages
    test_name=$render_test
    test_children=$render_children
    assertion='empty page lost its plain restart link'
  fi
  test_status=0
...
run_green restored
echo 'back-office pagination sources restored exactly; named scoped tests passed.'
