...
CASES = [{'name': 'enqueue',
 ...
 {'name': 'revoked-recipient',
  'path': 'internal/twofactor/query.sql',
  'before': "WHERE id = $1 AND role IN ('staff', 'admin');",
  'after': 'WHERE id = $1;',
  'test': 'TestStaffInvitationCommitsWithANewGrantWithoutRecipientPII',
  'marker': 'revoked invite still has recipient="changed-invitation-',
  'package': './internal/twofactor',
  'integration': True,
...
os.chdir(repo)
proof = Path(os.environ.get("RUNNER_TEMP", tempfile.gettempdir())) / "goen-issue-232-mutations"
