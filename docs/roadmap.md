...
**現況**:退貨停在「同意」。schema 允許 `approved → completed`,應用程式只做得到
approve / reject;結案不寫任何庫存異動,所以**退回來的貨永遠回不到架上**。
`inventory_movements` 的 `return` reason 有 CHECK、有安全庫存豁免、有後台顯示文字
—— 四個地方都寫好了,沒有任何呼叫者。

**要什麼**:收件 → 驗貨 → 處置(可售 / 拆封品 / 瑕疵)→ 回架 → 結案,以及退款
停在 pending 時的收尾door。逐行數量,不是整筆。

**為什麼是第一名**:它是唯一一個「商業迴圈沒有閉合」的缺口,而且不依賴任何外部
整合 —— 帳本、狀態機、稽核都已經在了。

### A2. 分批出貨

**現況**:`order_shipments` / `order_shipment_lines` 模型了多包裹、逐行數量、
包裹與行的複合鍵,而 `Ship` 一次寫掉所有未出貨的行、消耗所有保留。`CanShip` 只認
`picking`,狀態機不會走回 picking —— 所以**一張訂單這輩子最多一個包裹**。

**要什麼**:部分出貨的輸入、逐行保留結算、多次出貨通知,以及「完成是否蘊含送達」
這個問題的答案(`shipped → completed` 現在會蓋送達戳記,見 CLAUDE.md)。

**為什麼**:整套 schema 存在卻到不了,是這個 repo 自己的
`TestEveryTableHasAWriter` 抓的那個形狀,只是從應用程式那一側。
