...
- Go 1.27.0 · `net/http` routing, no framework
...
- `golangci-lint` v2.13.2, zero tolerated findings
...
port, and `exhaustruct_v5` is enabled for exactly one type — `i18n.Message` — so a
...
14. **Trusting a Stripe webhook for anything but "what happened".** Which
    ...
    A signature-verified webhook is the automatic posting door. The only other
    one is an explicit audited admin attribution after checking a complete
    Session at Stripe; it uses the immutable intent and the same capture guards.
...
2. **Money and privileged writes go through `SECURITY DEFINER` functions and a
   ...
   execute only the explicitly granted set. Its privileged write doors include
   `record_inventory_movement`, `hold_inventory`,
   `consume_reservation`, `release_reservation`,
   `open_payment`, `capture_payment`, `record_expired_payment`, `cancel_payment`,
   `mark_payment_event_unreconciled`, `lock_cart_catalogue`, `post_store_credit`,
...
goen's data rules live in the schema, not only in Go: 247 CHECKs, 82 foreign
keys, 64 unique indexes and 42 rule triggers, measured from `pg_constraint`,
...
A cart is an aggregate too. Every writer locks its `carts` row before touching
`cart_items`; operations spanning two carts lock their UUIDs in ascending order.
Checkout's order is stricter: idempotency advisory key, cart row, all product
variants and products in UUID order, the customer's credit account, then the
coupon definition before the publication/price/availability snapshot and quote
gate. Variant-before-product matches catalogue integrity triggers; the product
lock makes retirement linearizable with a stale-cart POST; the cart lock makes
`CartLines → ClearCart` linearizable with add, edit, reorder and account
adoption. Every multi-variant stock writer — checkout, cancellation and return
restocking — uses that same UUID order. Credit and coupon locks go through
narrow `SECURITY DEFINER` doors: `store` retains no table UPDATE privilege merely
because PostgreSQL requires it for `FOR UPDATE`.
Inventory hold and release both take order before variant; release takes order
before the reservation row too, matching cancellation and preventing expiry or
re-hold from closing an order↔reservation/variant cycle.
...
| `make verify` | The gate, no Docker: fmt-check → templ-check → squawk → sqlc-check → vet → deadcode → lint → production-build-check → integration-build-check → test-race |
...
between them decrements anything. Cart's private `holdTTL` is `payWindow` plus
the Stripe session floor + the start margin — 60 minutes, written as a sum
so the 29-minute start window, Stripe's 30-minute floor, and the one-minute
transport/rounding margin cannot drift into the equality that once let a
session outlive the stock it was paying for.

**The order must equal the last quote the customer actually saw.** The form
carries one fixed-size checkout quote identity, not cart JSON or individually trusted
money fields. Its canonical SHA-256 identity covers the cart ID; every variant,
quantity and unit price; shipping version and charged amount; normalized coupon
and discount; and exact store-credit spend. Line order is presentation-only.
`PlaceOrder` rebuilds those facts from transaction-bound queries under the locks
above and compares before creating the order, holding stock, spending credit or
redeeming the coupon. Thus an equal grand total made from different goods or
discounts is still stale. The digest is an equality witness, not an authorization
credential. A committed retry is found by `(cart, idempotency key)` before the
now-empty cart is examined; the key is globally owned, so another cart cannot
silently reuse it and create an untracked order.
...
the automatic paid fact. A provider-complete Session with no applicable event
can only be posted by an explicit audited staff outcome through the same capture
guards. The customer's return to `success_url` is a page, not a fact.
`GOEN_STRIPE_API_KEY` may be empty (the site still
...
so the abandoned path never runs; sixty minutes on, `ExpiredReservations` matches
the order and `release_reservation` puts the units back; days later
`async_payment_succeeded` lands. The provider has taken money, but the released
reservation is now positive, durable evidence under the order lock:
`capture_payment` refuses `payments_capture_refuses_released_stock`, the webhook
commits a `refused_capture` alarm, and the back office exposes only the refund
resolution. It cannot become a paid order with an empty held-reservation slice.

That remains mistake #13's shape a second time at the provider boundary: money
arrived after the stock promise ended. The database now fails financially safe —
local fulfilment is refused and the money must be returned — but an automatic
refund alarm is not support for a delayed method.
Card is what the bounded Checkout Session and 60-minute stock hold can survive,
so card is what goen offers; Apple Pay and Google Pay ride on that type. Supporting a delayed method is a FEATURE and
...
`payment_webhook_events.reconciled_at` is the operator's explicit safe-release
statement: every provider-side cent was fully refunded, or a succeeded local
payment already accounts for it. Merely checking a version or deciding an
attribution is not enough to lift the payment gate. Those outcomes happen at
Stripe and goen cannot see them land. Without the acknowledgement `/admin/health` is
...
There is a second, deliberately typed door for a provider-complete Session whose
webhook outcome has not arrived (or whose understood completed event was unpaid):
the payment is `requires_reconciliation` and appears on the same health page.
Staff must choose one of two financially opposite facts. Confirmed paid posts the
row's immutable intent through `capture_payment`, then appends the paid event,
loyalty award, receipt and audit in one transaction; confirmed unpaid or fully
refunded moves it to terminal `reconciled` and alone permits a new generation.
Stripe `complete` itself proves neither. When an unreconciled event exists, this
payment-level door refuses; the event path conservatively requires refund or an
already-succeeded accounting rather than guessing from a refused payload.
...
That outcome now has FOUR canonical durable causes in the existing reason
...
Checkout Session with no local payment row, `cancelled_order_capture` for
money against a cancelled order, and `refused_capture` for verified money a
stable local amount, state, or schema invariant will not post. The read side is a tri-state — ignored type,
...
ERROR and is not also marked unreadable. All four unreconciled causes answer
200: retrying replays the same bytes and cannot repair missing attribution,
version skew, a settled order, or a deterministic local refusal, while the
durable row gives a person the event and object references to act on. A linked,
unreconciled event also blocks another Checkout Session for that order until a
person records the explicit safe-release conclusion; an alarm without that gate
permits a second charge while the first payment still waits for refund.

**A Stripe idempotency generation ends only in a recorded provider fact.** A
Session can be created successfully while `open_payment` then refuses because
the order was cancelled, funded, repriced, or gained an unreconciled event
during the network call. Every local admission error therefore expires the
remote Session with a short context detached from the browser. Stripe-confirmed
expiry writes a cancelled tombstone. If expiry loses a race to `complete`, a
fresh Retrieve writes `requires_reconciliation` instead; its row consumes the
generation without inventing either cancellation or capture, and a later
webhook or audited operator resolution converges it. After a successful
open, and whenever an old-amount Session is retired, the handler does a fresh
Retrieve: an idempotent Create may replay its original `open` response after
the real Session is expired. `expired` converges the local row to cancelled;
`complete` never does, because its paid webhook may still be in flight. A
transaction advisory lock keyed by provider/ref is the first lock in webhook,
open, terminal recovery, and reconciliation paths. Thus a webhook racing the
local open either sees the payment, or commits first and its immutable event
history makes `open_payment` refuse that ref forever — reconciliation permits a
new Stripe identity, never late-linking the observed one. The
capture and open functions both acquire order before payment locks, so the
database cannot manufacture an order↔payment lock cycle at that boundary.
`ExpiredReservations` and `release_reservation` also treat that reconciliation
state as a stock pin for non-cancelled orders. Under the same order lock, paid
attribution first makes the order committed and keeps its goods for fulfilment;
the unpaid/refunded outcome removes the pin so an already-expired hold returns
on the next sweep.

There is an earlier boundary where no local reconciliation state can exist yet:
Stripe may complete a Session just before its deadline, while the signed webhook
or a browser Retrieve reaches goen only after the stock sweeper. Pinning every
`requires_payment` row would avoid that race but a lost expiry webhook would then
hold stock forever. Instead, `release_reservation` and `capture_payment` share the
order lock and a released reservation is an irreversible capture fence. If the
capture owns the order first, it becomes committed and release is refused. If the
sweeper owns it first, the late verified capture becomes a durable
`refused_capture`; manual paid attribution is refused by the same constraint and
the only honest outcome is a provider refund followed by safe release. This is a
deliberate availability tradeoff at the deadline: goen may refund a real payment
rather than promise goods that were already returned to sale.
...
**Erasure follows proved ownership, never a payload substring.** Registration
accepts an unproved address, as does a pending address change. Either may name a
victim, so neither authorises deleting every newsletter, contact, restock request
or queued letter addressed to that mailbox. `erase_user` always removes messages
whose identity is tied to the account — reset and verification token digests,
its order numbers, and its user-bound restock notification ids. Only a proved
current address opens the wider address-keyed cleanup.

The outbox recipient comparison is an exact, case-insensitive JSON value match,
with both the current `email` tag and the legacy Go `Email` key understood. `%`
and `_` are legal mailbox characters but SQL `ILIKE` wildcards; treating the
address as a pattern let one account erase unrelated customers' queued mail.
Likewise, a pending verification removes only its own `account.email_verify`
dedupe key. Two accounts may be proving the same mailbox, and neither owns the
other one's intent yet.

**`stock_notifications` is also both user- and address-keyed.** A signed-in
request has a user id and is removed by that stable identity. A signed-out guest
request has only an address and is swept with newsletter/contact data only after
that mailbox is proved. The fixtures hold user-bound, verified-address and
unproved-victim shapes separately; counting rows by a user id after its foreign
key became NULL was the false-green test this distinction replaced.
...
- **Delayed payment methods are OFF, and turning them on is a feature with a
  ...
  deadline rather than a flat hold duration, an `ExpiredReservations` predicate
