...
func TestEveryColumnIsReadOrWritten(t *testing.T) {
	allowed := map[string]string{
		...
		"payment_provider_links.created_at":     "a row-birth timestamp its own DEFAULT writes; no query shows it",
		"payment_dispute_events.recorded_at":    "a row-birth timestamp its own DEFAULT writes; no query shows it",
		"payment_dispute_movements.id":          "a surrogate primary key; its own constraint is the use, and that is inside the block this guard cuts",
		"payment_dispute_movements.recorded_at": "a row-birth timestamp its own DEFAULT writes; no query shows it",
	}
