...
// Package icons holds goen's inline SVG icons. They are inline rather than

// <img> or a sprite so they inherit currentColor and the design system's accent
...
// Mark is the goen brand mark: two rings meeting.
...
// Star is the rating glyph. Stroke-only: the design system's rating component
// carries the fill/empty distinction in colour, not in shape.
...
// Category renders the tile glyph for a category's icon_key. The keys are
// database values, so an unrecognised one renders nothing rather than panicking.
